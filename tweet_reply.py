...
def respondToTweet(file='tweet_ID.txt'):
    ...
    for mention in reversed(mentions):
        ...
        if 'magnolia fact' in mention.full_text.lower():
            logger.info("Responding back with fact to -{}".format(mention.id))
            try:
                tweet = get_fact()
                logger.info("liking and replying to tweet")

                api.create_favorite(mention.id)
                api.update_status('@' + mention.user.screen_name + " Here's your Magnolia tree fact: ", 
